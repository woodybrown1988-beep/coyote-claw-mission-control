...
test('registry: Reports nav order ends …reservations, labour, costs; the costs icon is the ruled path; server.js requires the page after reservations', () => {
  ...
  assert.deepEqual(reports.items.map((i) => i.key), ['revenue', 'report-library', 'rota-review', 'reservations', 'labour', 'costs', 'inventory', 'kitchen-safety', 'operations'], 'Reports order — costs after labour');
