...
test('registry: Reports nav order ends …reservations, labour, costs; the costs icon is the ruled path; server.js requires the page after reservations', () => {
  ...
  assert.deepEqual(reports.items.map((i) => i.key), ['revenue', 'labour', 'costs', 'reservations', 'operations', 'inventory', 'stock', 'customer-growth', 'kitchen-safety', 'report-library', 'files'], 'Reports order — costs after labour');
