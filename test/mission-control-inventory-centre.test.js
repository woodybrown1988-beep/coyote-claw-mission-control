...
test('registry + nav: inventory in the Reports group AFTER costs; server requires the page; contract', () => {
  ...
  assert.deepEqual(reports.items.map((i) => i.key), ['revenue', 'report-library', 'rota-review', 'reservations', 'labour', 'costs', 'inventory', 'kitchen-safety', 'operations', 'customer-growth'], 'inventory lands after costs');
