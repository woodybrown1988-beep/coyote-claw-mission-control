...
test('registry: the Reports group order = revenue, report-library, rota-review, reservations; route + ico pinned', () => {
  ...
  assert.deepEqual(reportsGroup.items.map((i) => i.key), ['revenue', 'report-library', 'rota-review', 'reservations', 'labour', 'costs', 'inventory', 'kitchen-safety', 'operations'], 'Reservations lands AFTER Rota Review (Labour joined the group — centre L1)');
