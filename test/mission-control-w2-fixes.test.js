...
test('nav restructure (page-map amendment 2026-07-21): Reports section groups Revenue + Library + Rota Review; old paths 308 to the new home with query preserved', () => {
  ...
  assert.deepEqual(reportsGroup.items.map((i) => i.key), ['revenue', 'labour', 'costs', 'reservations', 'operations', 'inventory', 'customer-growth', 'kitchen-safety', 'report-library', 'files'], 'Revenue + Library + Rota Review + Reservations + Labour (centre L1 2026-07-21), in order');
