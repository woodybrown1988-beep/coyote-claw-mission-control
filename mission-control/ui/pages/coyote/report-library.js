...
module.exports = {
  ...
  render(section, ctx) {
    ...
    return {
      ...
      body: styles + (viewer ? viewer : '') + `<div class="sec-label">Reports <span class="mono">(${m.sort === 'department' ? 'by department, newest within' : 'newest first'})</span><span class="rule"></span></div>` + bar + `<div class="rl-list">${items}</div>`,
    };
  },
};
